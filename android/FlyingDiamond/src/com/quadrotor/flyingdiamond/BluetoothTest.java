...
public class BluetoothTest extends Activity {
	...
	private char END_COMMAND = '\r';
	private char DELIMITER = ' ';
	
	private int x_accel = 2;
	...
	public synchronized void connected(BluetoothSocket socket,
		...
		//timer.scheduleAtFixedRate(askForAccelTask, 0, 500);
	}
	...
	private void update_accel_text() {
		...
		Log.v(TAG, "accelText: " + accelText.getText().toString());
	...
	private void process_one_arg_command(String command, int arg) {
		...
		if (command.equals("ACC_X")) {
			x_accel = arg;
			update_accel_text();
		} else if (command.equals("ACC_Y")) {
			y_accel = arg;
			update_accel_text();
		} else if (command.equals("ACC_Z")) {
			z_accel = arg;
			update_accel_text();
		}
	}
